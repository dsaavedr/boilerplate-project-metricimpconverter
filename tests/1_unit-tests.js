    ...
    suite("Validate outputs", () => {
        ...
        test("convertHandler should correctly convert L to gal.", () => {
            assert.equal(
                ...
                parseFloat(1 / 3.78541).toFixed(6),
                "L to gal"
            );
        });
        ...
        test("convertHandler should correctly convert km to mi.", () => {
            assert.equal(
                ...
                parseFloat(1 / 1.60934).toFixed(6),
                "km to mi"
            );
        });

        test("convertHandler should correctly convert lbs to kg.", () => {
            assert.equal(convertHandler.handle("lbs").returnNum, 0.453592, "lbs to kg");
        });

        test("convertHandler should correctly convert kg to lbs.", () => {
            assert.equal(
                convertHandler.handle("kg").returnNum,
                parseFloat(1 / 0.453592).toFixed(6),
                "kg to lbs"
            );
        });
    });
